...
系统实时显示当前天气：42°F（7.2°C），体感温度 37.9°F，湿度 94%。

![Risk Overview - Cold Day](https://raw.githubusercontent.com/ipandas111/NYC_HeatWave_Project/main/images/case1-risk-overview.png)
...
所有社区均为绿色（低风险），包括：
- Flushing: 20.9
- Downtown Brooklyn: 23.2
- Lower East Side: 25.0
- Parkchester: 24.7

---

### 场景二：AI 风险分析（Mott Haven 社区）

用户选择特定社区后，AI 自动生成详细的风险分析报告。

![AI Analysis - Mott Haven](https://raw.githubusercontent.com/ipandas111/NYC_HeatWave_Project/main/images/case2-ai-analysis.png)
...
**风险因素分解：**
- 温度风险：5.5/30
- 人口脆弱性：24.6/30（老年人口 18%、贫困率 38%、空调覆盖率 45%）
- 环境因素：12/40（绿地覆盖率 8%）

**AI 建议：**
> "继续对老年居民进行常规福利检查，监测天气预报变化，确保如果情况升级则开放避暑中心。"
...
└── core/
    ...
    ├── real_data_loader.py # Open-Meteo 天气 API
    └── email_alert.py      # 自动邮件告警
```
...
| 字段 | 说明 | 示例 |
|------|------|------|
| neighborhood | 社区名称 | Mott Haven |
| zip_code | 邮编 | 10451 |
| borough | 行政区 | Bronx |
| latitude | 纬度 | 40.8075 |
| longitude | 经度 | -73.9442 |
| elderly_pct | 65岁以上老人比例(%) | 18 |
| ac_pct | 空调覆盖率(%) | 45 |
| poverty_pct | 贫困率(%) | 38 |
| green_space_pct | 绿地覆盖率(%) | 8 |
...
| temperature | 温度(°F) | 95 |
| humidity | 湿度(%) | 65 |
| heat_index | 体感温度(°F) | 108 |
| risk_score | 风险分数 | 82 |
| risk_level | 风险等级 | Critical |
